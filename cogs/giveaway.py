...
class Button(discord.ui.View):
    ...
    async def Join_Button(self,interaction:discord.Interaction,button: discord.ui.Button):
        ...
        cur = await self.bot.db.execute("SELECT user_id FROM Giveaway_Entry WHERE message_id = ?", (interaction.message.id,))
        ...
        await self.bot.db.execute("UPDATE Giveaway_Running SET entries = entries + ? WHERE unique_id = ?", (1,int(interaction.message.id),) )
        await self.bot.db.commit()



        cur = await self.bot.db.execute("SELECT entries FROM Giveaway_Running WHERE unique_id = ?", (interaction.message.id,))
        res = await cur.fetchone()
        self.children[0].label=f"Entries: {res[0]}"
        await self.bot.db.execute("INSERT OR IGNORE INTO Giveaway_Entry (user_id, message_id) VALUES (?,?)", (interaction.user.id, interaction.message.id))
        await self.bot.db.commit()
...
class Giveaway(commands.Cog):
    ...
    async def start(self,ctx,GiveawayDuration:TimeConverter = None, winner = None, *,prize):
        ...
        message = await ctx.send(content="**__Giveaway Started__**",embed=embed,view=Button(self.bot),delete_after=600.0)
        await self.bot.db.execute("INSERT OR IGNORE INTO Giveaway_Running (unique_id,channel_id,prize,hostedby,total,running,entries,winners) VALUES (?,?,?,?,?,?,?,?)", (message.id,ctx.channel.id,prize,ctx.author.id,GiveawayDuration, 1,0, winner))
        await self.bot.db.commit()
    ...
    async def pause(self,ctx,giveaway_id = None):
        if giveaway_id != None:
            await self.bot.db.execute("UPDATE Giveaway_Running SET running = ? WHERE unique_id = ?", (0,int(giveaway_id),) )
            await self.bot.db.commit()

            cur = await self.bot.db.execute("SELECT unique_id,channel_id,prize,hostedby,total FROM Giveaway_Running WHERE unique_id = ?", (int(giveaway_id),))
            res = await cur.fetchone()

            ch = await self.bot.fetch_channel(res[1])
            ms = await ch.fetch_message(res[0])
            user = await self.bot.fetch_user(res[3])
    ...
    async def resume(self,ctx,giveaway_id = None):
        if giveaway_id != None:
            await self.bot.db.execute("UPDATE Giveaway_Running SET running = ? WHERE unique_id = ?", (1,int(giveaway_id),) )
            await self.bot.db.commit()
    ...
    async def reroll(self,ctx,giveaway_id = None):
        if giveaway_id != None:
            ...
            cur = await self.bot.db.execute("SELECT total FROM Giveaway_Running WHERE unique_id = ?", (int(giveaway_id),))
            res = await cur.fetchone()

            if not res:
                cur = await self.bot.db.execute("SELECT user_id FROM Giveaway_Entry WHERE message_id = ?", (int(giveaway_id),))
                res = await cur.fetchall()
                if res:
                    n = random.randint(len(res))
                    winner = await self.bot.fetch_user(res[n][0])
    ...
    async def running(self,ctx):
        cur = await self.bot.db.execute("SELECT unique_id,channel_id,prize,hostedby,total FROM Giveaway_Running WHERE running = ?", (1,))
        res = await cur.fetchall()
        ...
        if res:
            ...
            for x in res:
                host = await self.bot.fetch_user(x[3])
